...
def plot_snapshot_panel(
    ...
    sc = ax.scatter(
        lattice.edge_midpoints[:, 0],
        lattice.edge_midpoints[:, 1],
        c=values,
        s=55,
        cmap="viridis",
    )
...
def main() -> None:
    ...
    full_gap = (
        results_for_export["full_law"]["boundary_fraction"]
        - results_for_export["principal_branch"]["boundary_fraction"]
    )
    axes[1, 1].plot(
        results_for_export["full_law"]["time"],
        full_gap,
        linewidth=2.2,
        label="memory_gap = full_law - principal_branch",
    )
