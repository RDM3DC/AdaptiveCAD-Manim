...
class EdgeLattice:
    nx: int
    ny: int
    edge_pairs: Array          # shape: (E, 2, 2)
    edge_midpoints: Array      # shape: (E, 2)
    boundary_mask: Array       # shape: (E,)
    ...
    def square(cls, nx: int = 8, ny: int = 8) -> "EdgeLattice":
        """
        Build a square lattice with horizontal + vertical edges.
        Boundary edges live on the perimeter and are used as the topological
        recovery target in the benchmark.
        """
        if nx < 3 or ny < 3:
...
class ModelParams:
    ...
    pi_a_center: float = np.pi
    pi_a_min: float = 0.62 * np.pi
    pi_a_max: float = 1.25 * np.pi
    pi_a_sensitivity: float = 0.22
    ...
    recovery_target_fraction: float = 0.90
    seed: int = 7
...
class SimulationState:
    ...
    def copy(self) -> "SimulationState":
        return SimulationState(
            g=self.g.copy(),
            theta_R=self.theta_R.copy(),
            damaged=bool(self.damaged),
        )
...
def default_variants() -> list[VariantConfig]:
    return [
        ...
        VariantConfig(name="principal_branch", use_phase_lift=False),
        VariantConfig(name="no_topology_feedback", use_topology_feedback=False, chi_scale=0.0),
        VariantConfig(name="fixed_ruler", adaptive_ruler=False),
    ]
    ...
    if path.suffix.lower() in {".yml", ".yaml"}:
        try:
            ...
            raise RuntimeError(
                ...
                "Either `pip install pyyaml` or use JSON."
            ) from exc
...
def apply_overrides(params: ModelParams, overrides: dict[str, Any]) -> ModelParams:
    ...
    clean: dict[str, Any] = {k: v for k, v in overrides.items() if k in allowed}
...
def bulk_mean(values: Array, mask: Array) -> float:
    ...
    if not np.any(bulk):
        return 0.0
    return float(np.mean(values[bulk]))


def compute_local_chern_proxy(
    j_abs: Array,
    theta_used: Array,
    boundary_mask: Array,
) -> Array:
    """
    A bounded proxy for local topological support.
    This is intentionally a minimal executable surrogate, not a formal FHS Chern calculation.
    """
    eps = 1e-9
    ...
    if np.any(boundary_mask):
        boundary_phase_ref = np.angle(np.mean(np.exp(1j * theta_used[boundary_mask])))
    else:
        boundary_phase_ref = 0.0

    phase_alignment = 0.5 * (1.0 + np.cos(theta_used - boundary_phase_ref))
    c_loc = boundary_mask.astype(float) * np.clip(0.5 * phase_alignment + 0.5 * max(transport_gap, 0.0), 0.0, 1.0)
    return c_loc
...
def compute_currents(
    ...
    if np.any(boundary_mask):
        boundary_coherence = float(np.abs(np.mean(np.exp(1j * theta_used[boundary_mask]))))
    else:
...
def _step(
    ...
    damage_kick = p.damage_phase_kick * boundary_mask * (1.0 if state.damaged else 0.0) / (g_abs + 0.25)
    ...
    if variant.use_topology_feedback and variant.chi_scale != 0.0:
        c_loc = compute_local_chern_proxy(j_abs, theta_used, lattice.boundary_mask)
    else:
        c_loc = np.zeros_like(j_abs)
    ...
    dg_dt = (
        alpha * j_abs * np.exp(1j * theta_used)
        - mu * state.g
        - suppression
        + healing
    )
...
def simulate(
    ...
    *,
    ...
    """
    Deterministic minimal simulation harness for:
    - full recovery demo
    - matched-present ablations
    """
    if state is None:
